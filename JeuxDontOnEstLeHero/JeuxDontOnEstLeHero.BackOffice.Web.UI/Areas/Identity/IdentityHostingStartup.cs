            ...
            builder.ConfigureServices((context, services) => {
                services.AddDbContext<JeuxDontOnEstLeHeroBackOfficeWebUIContext>(options =>
                    options.UseSqlServer(
                        context.Configuration.GetConnectionString("AuthentificationtContext_Local")));

                services.AddDefaultIdentity<JeuxDontOnEstLeHeroBackOfficeWebUIUser>(options => options.SignIn.RequireConfirmedAccount = true)
